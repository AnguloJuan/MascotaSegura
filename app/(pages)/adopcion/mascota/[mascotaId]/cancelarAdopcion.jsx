...
export default function Cancelar({ idAdopcion, idAdoptante, idMascota }) {
    ...
    const handleCancelar = async () => {
        ...
        try {
            const params = JSON.stringify({ idAdopcion, idAdoptante, idMascota, motivo });
    ...
    return (
        <>
            <div className="d-flex flex-column w-100">
                <label htmlFor="motivo">Motivo de cancelación <span className='text-secondary fw-light'>(No es obligatorio)</span></label>
                <textarea name="motivo" id="motivo" rows="5" onChange={(e) => {e.preventDefault(), setMotivo(e.target.value)}} className='form-control w-100 rounded'></textarea>
                <button type='button' onClick={() => setWarningDialog(true)} className="btn btn-danger btn-lg mt-4" disabled={cancelando}>
                    Cancelar adopción
                </button>
            </div>
